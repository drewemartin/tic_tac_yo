class UsersController < ApplicationController
  ...
  def index
    ...
    if params[:search]
      ...
      @users = User.by_gender(params[:gender]).by_age_range(age_range[0],age_range[1]).by_city(params[:city]).everyone_but_current(current_user).order('users.created_at DESC').page(params[:page])
      # puts "run search"
      # puts User.by_gender(params[:gender]).by_age_range(age_range[0],age_range[1]).by_city(params[:city]).length
    else
      @users = User.order('users.created_at DESC').everyone_but_current(current_user).page(params[:page])
      # puts "run all"
